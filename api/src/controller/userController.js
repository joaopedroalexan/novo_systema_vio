...
module.exports = class userController {
  static async createUser(req, res) {
    ...
    const validationCPF = await validateCPF(cpf,null)
    if (validationCPF){
      return res.status (400).json(validationCPF)
    }
  ...
  static async loginUser(req, res) {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ error: "O E-mail e a Senha são obrigatórios para o login!" });
    }
    ...
    try {
      connect.query(query, [email], (err, results) => {
        ...
        return res
          .status(200)
          .json({ message: `Login Efetuado com Sucesso!`, user });
      });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ error: "Erro interno do Servidor" });
  ...
  const validationCPF = await validateCPF(cpf,null)
    ...
    const values = [name, email, password, cpf, id, data_nascimento];
    try {
